...
window.B2P = (function () {
  ...
  const FASES = [
    ...
    { id: 4, nombre: 'Pic', sub: 'Push / Pull / Legs ×5', semanas: [10, 11, 12], disco: 25, rpe: '8',
      fechas: '19 oct – 8 nov',
      objetivo: 'Stimulus maximal pour boucler la recomposition. {d} jours, mais des séances de {min} minutes, pas de 2 heures. RPE 8 : 1-2 répétitions en réserve sur les dernières séries.' }
  ];
  ...
  const HITOS_SEMANA = {
    ...
    10: { t: 'Un jour de plus', d: 'Première semaine du nouveau bloc : fais UNE série de moins sur tout. Ajouter un jour d’entraînement est le point de risque tendineux maximal du plan ; on y entre en marchant, pas en sautant.' }
  };
  ...
  /* ---------- LES 8 RÈGLES ---------- */
  const REGLAS = [
    { n: 1, t: 'RPE contrôlé', d: 'Chaque phase a son plafond d’effort. Ton système nerveux se souvient d’avoir été athlète ; tes tendons ont passé des années sans charge. Freine avant qu’ils ne freinent.' },
    ...
    { n: 8, t: 'Le minimum non négociable', d: 'Le schéma qui tue, c’est 3 mois à fond / 3 à zéro. La semaine chaotique a un plancher : 2 forces + 1 cardio. Ça maintient tout.' }
  ];
  ...
  const CIENCIA = {
    ...
    temas: [
      ...
      { t: 'Diet break', d: 'Alterner déficit et pauses à maintenance a atténué la chute métabolique et amélioré la perte de gras dans l’étude MATADOR. Sur {s} semaines, sa valeur principale est ailleurs : il t’apprend qu’une semaine d’arrêt AVEC un plan n’est pas une rechute.', ref: 'Byrne 2018 (Int J Obesity, MATADOR)' },
      ...
      { t: 'La décharge bien faite', d: 'Tout arrêter une semaine coûte de la force ; ce qui marche, c’est couper le volume de moitié en gardant le poids sur la barre. D’où la semaine 9 en décharge OBLIGATOIRE de ce type.', ref: 'Coleman 2024 (PeerJ, ECR de décharge)' },
      ...
      { t: 'La santé d’abord', d: 'Après des années sans activité vigoureuse, avant le travail dur de F3-F4 : tension artérielle et bilan basique (lipides, glucose/HbA1c). Avec des symptômes de quelque type que ce soit, médecin avant de continuer.', ref: 'ACSM Preparticipation Health Screening' }
    ]
  };
  ...
  const UI = {
    ...
    quizAfinara: 'Ça affinera ton plan.', quizTitulo: 'Tes goûts', quizPista: 'Glisse : à droite j’aime, à gauche non',
    ...
    gen: { kcalHueco: 'Ce menu totalise ~{m} kcal par jour et ton objectif est {k} : {q}. Les recettes ont une taille fixe ; ajuste le riz, les pâtes ou le pain du repas principal.', kcalSube: 'il te manque ~{d}', kcalBaja: 'tu dépasses de ~{d}', chkDentro: 'dans le couloir', chkBajo: 'en dessous', chkAlto: 'au-dessus', lChkD2: 'Poids dans le couloir à la semaine {s}.', tHombroT: 'Épaule · coiffe et dentelé', tHombroD: 'Rotation externe à la bande 2×15 par côté et élévation en Y allongé 2×12, lentes. Avant toute poussée et les jours creux. La coiffe ne gagne pas avec du poids : elle gagne avec du contrôle.', tHombroW: 'À la maison avec une bande, ou l’haltère le plus léger que tu aies.', descargaSinBarra: 'Même routine avec la moitié des séries et la même charge. Pas un arrêt : entretien du tissu et vacances pour tendons et articulations.', r2SinBarra: 'D’abord monte les répétitions dans la fourchette, ensuite la charge (le plus petit saut dont tu disposes : un haltère de plus, une bande plus dure ou une variante plus difficile). Seulement si la technique était propre sur TOUTES les séries. L’app te le suggère.', protHueco: 'Le menu fournit ~{m} g de protéines par jour ; jusqu’à tes {p} g, le pont ce sont les prises extra (un shake ou une portion de plus).', finRecapT: 'Ton bloc, en chiffres', subCorporal: 'Version au poids du corps : atteins proprement le haut de la fourchette, puis passe à la variante supérieure.', subRepe: 'Deuxième tour : sans matériel il n’y a pas trente variantes, et répéter le mouvement proprement construit quand même.', f2nCasa: 'Mise en charge', f2oCasa: 'Réapprendre les basiques avec haltères et bandes et construire une base de charge. Travaille à 65-70% de ce que tu sens possible, avec 3 répétitions en réserve TOUJOURS.', f2nNada: 'Progression au poids du corps', f2oNada: 'Maîtriser les progressions avec ton propre corps et construire une base. Le levier monte avant les répétitions : variante plus dure seulement avec une technique propre.', gemNota: 'Le mollet lent est l’assurance du tendon : ne le saute pas.', tendonSinTrote: 'La force revient en semaines ; le tendon demande des mois (son collagène se renouvelle ~10× plus lentement et n’a pas de mémoire musculaire). Ce bloc est l’assurance du plan : il commence en semaine 1 et accompagne tout le bloc.', introNunca: 'Plan vérifié contre l’évidence (méta-analyses et essais 2010-2025). L’idée qui ordonne tout : en partant de zéro on progresse vite, les premiers mois apportent les plus grands gains de force de ta vie, mais le tissu conjonctif traîne derrière le muscle. Voilà pourquoi les charges montent lentement même quand tu pourrais plus.', introActivo: 'Plan vérifié contre l’évidence (méta-analyses et essais 2010-2025). L’idée qui ordonne tout : qui s’entraîne déjà n’a pas besoin de plus de punition mais d’une meilleure dose. Le volume juste, la progression notée et le repos compté séparent se maintenir de progresser.', cNuncaT: 'Partir de zéro', cNuncaD: 'La première année apporte les plus grands gains de force d’une vie : presque toute dose bien faite fonctionne, d’où l’inutilité des programmes extrêmes. La technique d’abord : les répétitions propres d’aujourd’hui sont les kilos sûrs de dans trois mois.', cNuncaR: 'gains de débutant : revues ACSM et méta-analyses dose-réponse', cActivoT: 'Ajouter sans se casser', cActivoD: 'Le risque de qui s’entraîne déjà est d’empiler du volume neuf sur l’ancien. Les sauts au-dessus de ~1,3× ta charge moyenne récente font exploser les blessures : ajoute une variable à la fois (jours, volume ou intensité), jamais les trois.', cActivoR: 'consensus CIO sur la charge d’entraînement (ACWR)', cSupT: 'Un surplus qui construit', cSupD: 'Construire du muscle demande un petit surplus (~250-350 kcal) : au-dessus, l’extra part vers le gras. La balance doit monter lentement ; si elle monte vite ce n’est pas du muscle, la synthèse protéique a un plafond hebdomadaire.', cSupR: 'Garthe 2013 · Slater 2019 (surplus et composition)', r1Nunca: 'Chaque phase a son plafond d’effort. En partant de zéro, la force monte plus vite que la résistance de tes tissus : laisse toujours 2-3 répétitions en réserve et les gains arrivent quand même, sans péage.', r1Activo: 'Chaque phase a son plafond d’effort. Tu arrives en t’entraînant, mais ce volume est nouveau : respecte les plafonds de RPE les deux premières semaines, monte ensuite. Freiner à temps, c’est ce qui te laisse progresser les {s} semaines.', r8Nunca: 'L’ennemi du début n’est pas la dureté, c’est l’irrégularité. La semaine chaotique a un plancher : 2 forces + 1 cardio. Ça maintient tout en marche.', r8Activo: 'Qui s’entraîne a aussi des semaines impossibles. Le plancher : 2 forces + 1 cardio. Avec ça rien ne se perd ; le reste se récupère.', f1nNunca: 'Fondations', f1oNunca: 'Construire l’habitude et apprendre les patrons de mouvement sans punir les articulations. Rester sur sa faim est intentionnel.', f2nNunca: 'Technique', f2oNunca: 'Apprendre les basiques avec charge légère : chaque répétition propre maintenant, ce sont des kilos sûrs après. Travaille loin de l’échec TOUJOURS.', f3oNunca: 'Volume et intensité pour de vrai, la technique désormais rodée. Termine chaque série capable de faire 2 répétitions de plus, des vraies.', f1nActivo: 'Base', f1oActivo: 'Deux semaines d’adaptation au plan : dose connue, registre en route et technique affûtée avant de monter quoi que ce soit.', f2nActivo: 'Construction', f2oActivo: 'Volume progressif sur ta base : travaille à 70-75% de ce que tu sens possible, avec 2-3 répétitions en réserve.', f3oActivo: 'Volume et intensité réels pour forcer le changement. Termine chaque série capable de faire 2 répétitions de plus, des vraies.', cierrePerder: 'Le vrai but du plan n’est pas le {f} : c’est d’y arriver en t’entraînant par habitude, sans cycle on/off. Le poids qui descend est la conséquence, pas le but.', cierreRecomp: 'Le vrai but du plan n’est pas le {f} : c’est d’y arriver avec l’habitude installée et les vêtements qui tombent autrement. La recomposition est lente par design : la constance est le but.', cierreGanar: 'Le vrai but du plan n’est pas le {f} : c’est d’y arriver plus fort sous la barre, l’habitude installée. Le muscle se construit en mois : le bloc suivant commence où celui-ci finit.', cierreManten: 'Le vrai but du plan n’est pas le {f} : c’est que s’entraîner cesse d’être un plan pour devenir une coutume. Maintenir, c’est gagner.', cierreRenueva: 'Pour renouveler le bloc : Réglages, Créer / refaire mon plan. Deux touches et tu continues.', platoVegetariano: '3 œufs + 2 blancs, ou 250 g de skyr ou fromage blanc + whey, ou 200 g de tofu ferme, ou 150 g de tempeh, ou 250 g de légumineuses cuites + 1 œuf. Référence visuelle : une paume et demie.', platoVegano: '200-250 g de tofu ferme, ou 150-180 g de tempeh, ou 250 g de légumineuses cuites + une dose de protéine végétale, ou 80 g (sec) de soja texturé. Référence visuelle : une paume et demie.', suplVegT: 'Protéine végétale', suplVegD: '1 dose de protéine de pois ou de soja dans la prise pré-sommeil (et une autre là où la journée manque de protéines).', numRecomp: 'Déficit doux ~300-450 kcal/jour : recomposer demande de la patience, pas de l’agressivité.', numSup: 'Surplus ~250-350 kcal/jour : plus, ce n’est pas plus de muscle, c’est plus de gras (Garthe 2013).', numMan: 'Ta maintenance estimée : la moyenne hebdo juge et ajuste.', ritmoSubeT: 'Rythme de prise attendu', ritmoManT: 'Rythme attendu', ritmoSubeN: '≈0,25% du poids/sem : ce que le muscle peut réellement construire. Moyenne hebdo, pas au jour le jour.', ritmoManN: 'La moyenne hebdo doit rester à ±0,3 kg de ton départ.', wjN1: 'Marche-course I', wjN2: 'Marche-course II', wjN3: 'Marche-course III', lChkN: 'Checkpoint S{s}', lChkD: 'Poids dans le couloir (ou mieux) à la semaine {s}.', alRapidoBaja: 'Ajoute 150 kcal de glucides. À ce rythme, le déficit mange aussi du muscle.', alLentoBaja: 'Vérifie portions et pas pendant deux jours avant de couper quoi que ce soit ; si ça reste plat, retire 100 kcal de glucides les jours de repos seulement.', alRapidoSube: 'Tu montes plus vite que le muscle ne se construit : retire 150 kcal de glucides pour que le surplus ne soit pas du gras.', alLentoSube: 'Le surplus ne se voit pas sur la balance : ajoute 150 kcal de glucides les jours d’entraînement.', alMantenT: 'Tu dérives de la maintenance', alMantenD: 'Deux semaines de dérive d’affilée : ajuste 100-150 kcal dans la direction opposée et ne touche pas à l’entraînement.', circProg: 'Ajoute 1-2 répétitions par semaine là où ta technique reste propre : c’est ça, la progression.', durAprox: '≈{m}′', splitFbC: 'Full Body', splitTpC: 'Haut · Bas', splitPplC: 'Push · Pull · Legs', faseSub: '{s} ×{d}', nf1: 'F1–F2 (sem 1-{a})', nf2: 'F3 (sem {b}-{c})', nf3: 'F4 (sem {d}-{e})', dietBreakNota: 'Semaine {w} : DIET BREAK à ~{k}', hitoCribadoT: 'Bilan santé', hitoCribadoD: 'Avant la phase de charge, si tu es resté des années sans activité vigoureuse : tension en pharmacie et bilan basique (lipides, glucose). 15 minutes qui achètent la tranquillité.', hitoDietT: 'DIET BREAK', hitoDietD: 'Toute la semaine tu manges à la maintenance (~{k} kcal : +2 portions de glucides par jour, protéines inchangées). L’entraînement ne change pas. Ça restaure NEAT et leptine et casse le cycle on/off. Le lundi suivant, déficit à nouveau.', hitoDescargaT: 'DÉCHARGE (non optionnelle)', hitoDescargaD: 'Même routine avec la moitié des séries et le même poids sur la barre. Pas un arrêt : entretien du tissu et vacances pour tendons et articulations.', tomaNocheAlt: '+ chaque soir : prise pré-sommeil avec ta protéine végétale (soja ou pois), ~40 g en shake. ', franjaM: 'Tu t’entraînes le matin : petit-déj après la séance, pas avant.', franjaMd: 'Tu t’entraînes à midi : le repas principal tombe juste après la séance.', franjaT: 'Tu t’entraînes le soir : léger avant ; le dîner fait de repas post-entraînement.', cardioLibreT: 'Cardio : {d}', cardioLibreD: '{m}′ à un rythme confortable et régulier. Ton sport compte autant que le footing : la régularité commande.', chk1: 'Hors du couloir : vérifie portions et pas avant de toucher à quoi que ce soit. Les premières semaines, l’eau bouge aussi.', chk2: 'Deux semaines hors piste : ajuste 150 kcal de glucides dans la bonne direction. Les protéines ne bougent pas.', chk3: 'Clôture : photos, mesures et le bloc suivant, décidé avec des données.', lKgN: '−{v} kg', lKgD: 'Moyenne hebdo {v} kg sous ton départ.', lKgUpN: '+{v} kg', lKgUpD: 'Moyenne hebdo {v} kg au-dessus du départ. Du muscle, brique par brique.', lCintN: 'Taille −{v}', lCintD: 'Tour de taille sous {v} cm.', lReinaN: 'Métrique reine', lReinaD: 'Taille sous la moitié de ta stature : {v} cm.', lFinDesc: 'Plan de {s} semaines terminé. Le but était l’habitude ; le reste est conséquence.', marca: 'Plan généré pour toi', cuida: 'ménage : {a}', datos: '{p} kg · {a} cm · {e} ans', menuAviso: '{n} plats du menu ne collent pas à ton régime : remplace-les par n’importe lequel du livre de recettes, déjà filtré pour toi.', prepNota: 'Seules les recettes marquées « batch » se préparent le dimanche ; le reste se cuisine à la minute. Les quantités de courses comptent déjà les répétitions de la semaine.' },
    ...
    tour: { salta: 'Passer', sigue: 'Suivant', listo: 'À l’entraînement', otraVez: 'Revoir la visite de bienvenue',
      antes: ['Ton plan est lancé', 'Il commence le jour que tu as choisi. D’ici là, AUJOURD’HUI te montre les étapes préalables, et le menu est prêt pour que tu fasses tes courses.'],
      pasos: [
      ['Voici AUJOURD’HUI', 'Ta journée, déjà montée : la séance du jour avec ses exercices, séries et repos. Touche un exercice pour voir la technique et le muscle travaillé.'],
      ['Les repas du jour', 'Petit-déjeuner, déjeuner, dîner et la prise du soir, choisis pour ton objectif. Touche un plat et la recette entière s’ouvre.'],
      ['Ton journal quotidien', 'Poids, tour de taille et ce que tu as accompli. Un toucher par habitude, et tu clôtures la journée avec le bouton en bas. L’app tient le compte.'],
      ['Le plan entier', 'Un calendrier avec chaque phase dans sa couleur. Touche n’importe quel jour pour voir sa séance et ses repas. Glisse ou utilise les flèches pour changer de mois.'],
      ['Les quatre phases', 'De la réactivation au pic. Chaque bande indique ses semaines et son intensité ; déplie-la pour voir ce qu’elle vise.'],
      ['Ta bibliothèque d’exercices', 'Tous les mouvements par zone, avec le muscle en couleur, la technique et quoi utiliser s’il te manque du matériel.'],
      ['Ton objectif à table', 'Calories et macros de la phase en cours, et si le menu est en dessous ou au-dessus, de combien. Le pourquoi est dans Mon profil.'],
      ['Recettes', 'Petits-déjeuners, déjeuners, dîners et compléments, filtrés pour toi. Chaque recette avec photo, ingrédients en photo et pas à pas.'],
      ['Les courses de la semaine', 'Tout ce qu’il te faut, groupé par repas et avec la photo du produit pour le repérer en magasin. Coche ce que tu as déjà.'],
      ['Progrès honnêtes', 'Poids, taille, charges et régularité, en graphiques. Trop vite ? L’app te freine.'],
      ['Tes badges', 'Ils se gagnent par la régularité, pas par l’intensité. Chacun explique ce que tu as fait pour l’obtenir.'],
      ['Deux bulles', 'La loupe trouve n’importe quel exercice, plat ou réglage. La bulle de dialogue sert à me signaler bugs ou idées : je les lis tous.'],
      ['Mon profil', 'Tes réponses, refaire le plan, le partager par lien, exporter tes données, la confidentialité et supprimer le compte. Tout ce qui est à toi, là.'] ] },
    cuest: {
    ...
    ajRehacer: 'Créer / refaire mon plan', ajRehacerNota: 'Direction le questionnaire. Régénérer ne touche jamais tes registres quotidiens.', ajPeligro: 'Zone dangereuse', ajBorrar: 'Supprimer le profil et toutes les données', ajBorrarConfirma: 'Sûr ? Touche encore une fois pour TOUT effacer',
    ...
    rep: { t: 'Signaler', sub: 'Quelque chose cloche, ou une idée pour améliorer ça ? Dis-le ici.', bug: 'Ça ne marche pas', idea: 'Une idée', otro: 'Autre chose', txtL: 'Raconte', ph: 'Que s’est-il passé ? Si tu peux, dis sur quel écran tu étais et ce que tu attendais.', enviar: 'Envoyer', gracias: 'Bien reçu. Merci de l’avoir écrit.', corto: 'Écris-en un peu plus pour que ce soit compréhensible.', repRitmo: 'Attends une minute avant d’en envoyer un autre.', repCorto: 'Écris-en un peu plus pour que ce soit compréhensible.', errRed: 'Envoi impossible : vérifie ta connexion.', adjunta: 'Version, plateforme, langue, écran et taille sont joints pour pouvoir reproduire. Rien de ton plan, de ton poids ni de tes registres.' },
    nube: { correoL: 'E-mail', claveL: 'Mot de passe (8 minimum)', verClave: 'Afficher le mot de passe', ocultarClave: 'Masquer le mot de passe', previoT: 'Plan précédent sur cet appareil', previoTxt: 'Cet appareil gardait un plan d’avant l’existence des comptes. Il n’est pas repris tout seul, car il pourrait être à quelqu’un d’autre. S’il est à toi, récupère-le : il remplacera ce que tu as maintenant.', previoCta: 'Récupérer ce plan', previoOk: 'Plan récupéré', entrar: 'Se connecter', crear: 'Créer un compte', aCrear: 'Première fois ? Crée ton compte', aEntrar: 'Déjà un compte ? Connecte-toi', olvide: 'Mot de passe oublié', enviadoReset: 'E-mail envoyé : ouvre le lien pour le changer', nuevaClaveT: 'Choisis un nouveau mot de passe', guardarClave: 'Enregistrer le mot de passe', cambiada: 'Mot de passe changé : tu peux te connecter', confirmaCorreo: 'Vérifie ta boîte mail et confirme le compte, puis connecte-toi ici', yaExiste: 'Cet e-mail a déjà un compte : connecte-toi avec ton mot de passe', errCred: 'E-mail ou mot de passe incorrects', errCorreo: 'Écris un e-mail valide', errClaveCorta: 'Le mot de passe demande au moins 8 caractères', errRitmo: 'Trop de tentatives d’affilée : attends un instant', errRed: 'Pas de connexion au serveur : réessaie', local: 'Ton compte garde ton plan et te suit sur tous tes appareils. Toi seul peux le voir.', ajustesSub: 'BACK2PRIME · ton plan vit dans ton compte et toi seul peux le voir', cerrarSesionNota: 'Retour à la porte d’entrée. Ton plan reste dans ton compte : reconnecte-toi et reprends où tu en étais.' },
    comp: { t: 'Partager mon plan', nota: 'Crée un lien public en lecture seule avec ton plan : sans ton poids ni tes registres.', copiado: 'Lien copié', quitar: 'Ne plus partager', quitado: 'Lien désactivé', vT: 'Le plan de {n}', vSub: 'Généré avec BACK2PRIME', vCta: 'Fais le tien', noExiste: 'Ce lien n’existe pas ou son propriétaire l’a désactivé', sem: '{s} semaines', dias: '{d} jours/semaine' },
